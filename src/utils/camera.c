...
void init_pixel00_center(t_engine *e)
{
	...
	pixel_delta_dr = vec3_add_2inst_copy(e->cam->pixel_delta_down, e->cam->pixel_delta_right);
	pixel_delta_dr = vec3_div_const_copy(pixel_delta_dr, 2.0);
	e->cam->pixel00_loc_center = vec3_add_2inst_copy(e->cam->vp_upper_left, pixel_delta_dr);
}
